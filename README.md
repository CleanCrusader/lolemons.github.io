...
6. **Important — shipping speed**: that same log lists every delivery method configured in your Veeqo account (id, name, carrier). In Veeqo's own dashboard, check **Settings → Marketplace and Integrations** → your "Website (Custom Integration)" channel for whatever setting maps delivery methods to Amazon MCF shipping speed, and confirm which listed id actually corresponds to **Standard** shipping (not Expedited or Priority — those cost meaningfully more per order). Once confirmed, add that id as a new Cloudflare secret: `VEEQO_DELIVERY_METHOD_ID`. Checkout will fail with a clear error until this is set — that's intentional, so a wrong/missing mapping can't silently ship at a more expensive speed on every order.
